# ==== CONFIGURATION ====
$outputDir = "C:\Metrics"
$outputFile = Join-Path $outputDir "RAM_Processes.csv"
$sampleInterval = 5  # in seconds

# ==== Ensure Output Directory Exists ====
if (!(Test-Path -Path $outputDir)) {
    New-Item -ItemType Directory -Path $outputDir | Out-Null
}

# ==== Get System Info ====
$coreCount = (Get-CimInstance -ClassName Win32_Processor | Measure-Object -Property NumberOfLogicalProcessors -Sum).Sum

# ==== Initialize CSV Header if needed ====
if (!(Test-Path $outputFile)) {
    "Timestamp,Service Name,CPU Usage %,RAM Usage (MB)" | Out-File -FilePath $outputFile -Encoding UTF8
}

# ==== Function to Resolve Highest-Level Parent Name or Fallback ====
function Get-RootOrSelfName {
    param (
        [int]$procId,
        [hashtable]$procMap
    )

    $visited = @{ }
    $currentId = $procId

    while ($procMap.ContainsKey($currentId)) {
        if ($visited.ContainsKey($currentId)) { break }
        $visited[$currentId] = $true

        $parentId = $procMap[$currentId].ParentProcessId
        if (-not $parentId -or $parentId -eq 0 -or -not $procMap.ContainsKey($parentId)) {
            return $procMap[$currentId].Name
        }

        $currentId = $parentId
    }

    return $procMap[$procId].Name
}

# ==== Main Loop ====
while ($true) {
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

    try {
        # Build process map with parent-child relationships
        $processWmi = Get-CimInstance -Class Win32_Process | Select-Object ProcessId, ParentProcessId, Name
        $pidToParent = @{ }
        foreach ($proc in $processWmi) {
            $pidToParent[$proc.ProcessId] = @{
                ParentProcessId = $proc.ParentProcessId
                Name = $proc.Name
            }
        }

        # First snapshot
        $procStats1 = @{ }
        foreach ($p in Get-Process) {
            if ($p.CPU -ne $null) {
                $procStats1[$p.Id] = @{
                    CPU = $p.CPU
                    RAM = $p.WorkingSet64
                }
            }
        }

        Start-Sleep -Seconds $sampleInterval

        # Second snapshot
        $procStats2 = @{ }
        foreach ($p in Get-Process) {
            if ($p.CPU -ne $null) {
                $procStats2[$p.Id] = @{
                    CPU = $p.CPU
                    RAM = $p.WorkingSet64
                    Name = $p.ProcessName
                }
            }
        }

        # Calculate usage
        $usageData = @()
        foreach ($id in $procStats2.Keys) {
            if ($procStats1.ContainsKey($id)) {
                $cpuDelta = $procStats2[$id].CPU - $procStats1[$id].CPU
                $cpuPercent = [math]::Round(($cpuDelta / $sampleInterval) * 100 / $coreCount, 2)
                $ramMB = [math]::Round($procStats2[$id].RAM / 1MB, 2)

                $serviceName = Get-RootOrSelfName -procId $id -procMap $pidToParent
                if (-not $serviceName) {
                    $serviceName = $procStats2[$id].Name
                }

                $usageData += [PSCustomObject]@{
                    'Service Name'   = $serviceName
                    'CPU Usage %'    = $cpuPercent
                    'RAM Usage (MB)' = $ramMB
                }
            }
        }

        # Group by service name
        $grouped = $usageData |
            Group-Object 'Service Name' |
            ForEach-Object {
                $cpuSum = ($_.Group | Measure-Object 'CPU Usage %' -Sum).Sum
                $ramSum = ($_.Group | Measure-Object 'RAM Usage (MB)' -Sum).Sum
                [PSCustomObject]@{
                    'Service Name'   = $_.Name
                    'CPU Usage %'    = [math]::Round($cpuSum, 2)
                    'RAM Usage (MB)' = [math]::Round($ramSum, 2)
                }
            }

        # Top 3 by RAM usage
        $topUsage = $grouped | Sort-Object 'RAM Usage (MB)' -Descending | Select-Object -First 3

        # Log to CSV
        foreach ($entry in $topUsage) {
            "$timestamp,$($entry.'Service Name'),$($entry.'CPU Usage %'),$($entry.'RAM Usage (MB)')" |
                Out-File -Append -FilePath $outputFile -Encoding UTF8
        }

        # Feedback
        Write-Host "[$timestamp] Logged top 3 services by RAM usage (MB)." -ForegroundColor Cyan
    }
    catch {
        Write-Warning "[$timestamp] Error during tracking: $_"
    }
}
